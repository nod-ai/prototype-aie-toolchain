...
def build_cdo(elf_fp: Path, cdo_output_fp: Path):
    assert elf_fp.exists()
    elf_fp, cdo_output_fp = str(elf_fp), str(cdo_output_fp)
    col = 0
    ...
    startCDOFileStream(cdo_output_fp)
    FileHeader()

    if platform.system() != "Windows":
        XAie_ErrorHandlingInit(devInst)

    XAie_LoadElf(devInst, tile_0_2, elf_fp, False)
...
def build_xclbin():
    pi_elf = Path(__file__).parent.absolute() / "pi.elf"
    pi_cdo_fp = Path(__file__).parent.absolute() / "pi_cdo.bin"
    twopi_elf = Path(__file__).parent.absolute() / "twopi.elf"
    twopi_cdo_fp = Path(__file__).parent.absolute() / "twopi_cdo.bin"

    build_cdo(pi_elf, pi_cdo_fp)
    build_cdo(twopi_elf, twopi_cdo_fp)

    pi_bif_fp = Path(__file__).parent.absolute() / "pi.bif"
    with open(pi_bif_fp, "w") as f:
        f.write(bootgen.emit_design_bif([pi_cdo_fp]))
    pi_pdi_fp = Path(__file__).parent.absolute() / "pi.pdi"
    bootgen.make_design_pdi(str(pi_bif_fp), str(pi_pdi_fp))
    pi_pdi_spec = xclbinutil.pdi_spec(pi_pdi_fp, kernel_ids=["0x100"])

    twopi_bif_fp = Path(__file__).parent.absolute() / "twopi.bif"
    with open(twopi_bif_fp, "w") as f:
        f.write(bootgen.emit_design_bif([twopi_cdo_fp]))
    twopi_pdi_fp = Path(__file__).parent.absolute() / "twopi.pdi"
    bootgen.make_design_pdi(str(twopi_bif_fp), str(twopi_pdi_fp))
    twopi_pdi_spec = xclbinutil.pdi_spec(twopi_pdi_fp, kernel_ids=["0x101"])
    ...
    kernel_spec_pi = xclbinutil.kernel_spec("PI", kernel_id="0x100", buffer_args=["c0"])
    kernel_spec_twopi = xclbinutil.kernel_spec(
        "TWOPI", kernel_id="0x101", buffer_args=["c0"]
    )
    kernels_json_fp = Path(__file__).parent.absolute() / "kernels.json"
    with open(kernels_json_fp, "w") as f:
        json.dump(
            xclbinutil.emit_design_kernel_json([kernel_spec_pi, kernel_spec_twopi]),
            f,
            indent=2,
        )

    twokernels_xclbin_fp = Path(__file__).parent.absolute() / "twokernels.xclbin"
    xclbinutil.make_xclbin(
        str(mem_top_json_fp),
        str(aie_part_json_fp),
        str(kernels_json_fp),
        str(twokernels_xclbin_fp),
    )
...
device, kernel = init_xrt_load_kernel(
    Path(__file__).parent.absolute() / "twokernels.xclbin",
    "PI",
)
